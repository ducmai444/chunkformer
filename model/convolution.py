...
class ConvolutionModule(nn.Module):
    ...
    def forward_parallel_chunk(
        ...
        # crop to original chunk size to match residual length
        if lorder > 0:
            x = x[:, :, lorder:lorder + chunk_size]
        # mask batch padding for the center (valid) region
        if mask_pad.size(2) > 0:  # time > 0
            center_mask = mask_pad[:, :, lorder:lorder + chunk_size] if lorder > 0 else mask_pad
            x.masked_fill_(~center_mask, 0.0)
