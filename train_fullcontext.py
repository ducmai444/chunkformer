...
# Reuse existing utilities from train.py
from train import (
    TextTokenizer,
    AudioDataset,
    collate_fn,
...
def train_one_epoch_full(model, dataloader, optimizer, scheduler, device, scaler,
    ...
    for step, (xs, xs_lens, ys_cat, ys_lens) in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            # Full-context: set chunk_size=0 to signal full utterance if supported
...
def main():
    parser = argparse.ArgumentParser(description="Full-context pre-training for Chunkformer (CTC)")
